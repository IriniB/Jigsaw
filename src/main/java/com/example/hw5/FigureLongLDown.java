...
public class FigureLongLDown extends FigureType {
    public FigureLongLDown() {
        body = new Polygon();
        body.getPoints().addAll((double)x1, (double)y1);
    ...
    public void setPressedSquareNumber(MouseEvent mouseEvent) {
        ...
        if (cordX >= x1 && cordX <= x2 && cordY >= y3 && cordY <= y4) {
            squareNumber = 0;
            canMoveFigure = true;
        } else if (cordX >= x1 && cordX <= x2 && cordY >= y2 && cordY < y3) {
            squareNumber = 1;
            canMoveFigure = true;
        } else if (cordX >= x1 && cordX < x2 && cordY >= y1 && cordY < y2) {
            squareNumber = 2;
            canMoveFigure = true;
        } else if (cordX >= x2 && cordX < x3 && cordY >= y1 && cordY <= y2) {
            squareNumber = 3;
            canMoveFigure = true;
